...
export function usePopperOrigin(
    visible: boolean,
    ...
    useEffect(() => {
        ...
        if (boundingClientRect) {
            ...
            const popperBottom =
                ySpace > window.innerHeight + OFFSET ? ySpace - window.innerHeight + OFFSET : 0;
            const popperLeft = x < OFFSET ? OFFSET - x : 0;
            const popperRight =
                xSpace > window.innerWidth + OFFSET ? xSpace - window.innerWidth + OFFSET : 0;
