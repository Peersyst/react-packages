...
import {Toast, Row, ToastProvider} from "../../src";
...
<Canvas>
    <ToastProvider>
        <Row justifyContent="center"><ShowUncontrolledToast /></Row>
    </ToastProvider>
</Canvas>
...
{toastPosition.map((pos, index) => (
    <Button
      key={index.toString()}
      onClick={() =>
        showToast('Hello from ' + pos, { position: pos })
      }
    >
      {pos}
    </Button>
