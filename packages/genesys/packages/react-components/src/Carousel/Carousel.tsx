...
const Carousel = forwardRef((props: CarouselProps, ref): JSX.Element => {
    const {
        gap = 20,
        autoplay = false,
        ...
        style,
        children,
    } = useMergeDefaultProps("Carousel", props);
        ...
        <CarouselWrapper className={cx("Carousel", className)} style={style}>
            <CarouselRoot ref={handleCarouselRef} gap={gap} className="CarouselItems">
                {Children.map(children, (child, childIndex) =>
                    cloneElement(child, {
                        ...child.props,
                        ref: (r: HTMLElement) => childIndex === index && setRef(indexChildRef, r),
                    }),
                )}
            </CarouselRoot>
            {renderArrows && (
