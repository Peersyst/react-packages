...
import { ownerDocument } from "@peersyst/react-utils";
...
import {
    ...
    useForkRef,
    useIsFocusVisible,
} from "@peersyst/react-hooks";
import { EventHandlers } from "./required.types";
...
export default function useSlider(parameters: UseSliderParameters) {
    ...
    const [valueDerived, setValueState] = useControlledState({
    ...
    const createHandleHiddenInputChange =
        (otherHandlers: Record<string, React.EventHandler<any>>) => (event: React.ChangeEvent) => {
            ...
            if (marks && step == null) {
                newValue =
                    newValue < value ? marksValues[marksIndex - 1] : marksValues[marksIndex + 1];
            }
